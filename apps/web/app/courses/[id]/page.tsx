...
export default function CourseDetailPage() {
  ...
  // Use latestVersion from new schema
  const version = course.latestVersion;
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-6 pt-4">
              <div className="flex items-center gap-2 text-slate-500">
                ...
                <span className="text-sm font-bold">{version?.selfStudyDurationMinutes || 12} mins / Lesson</span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
                <Layers className="w-5 h-5" />
                <span className="text-sm font-bold">{version?.modules?.length || 0} Modules</span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              {version?.modules?.map((module: any, idx: number) => (
              ...
              {!version && (
                <div className="p-12 text-center bg-white rounded-2xl border-2 border-dashed border-slate-200">
                  <p className="text-slate-500">This course has no published content yet.</p>
                </div>
              )}
            </div>
          </div>
