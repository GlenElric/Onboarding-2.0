...
export default function AdminCourseDetailPage() {
  ...
  return (
    <div className="bg-surface min-h-screen px-8 py-10 max-w-5xl mx-auto font-body">
      <input ref={fileInputRef} type="file" accept=".pdf" className="hidden" onChange={handleFileChange} />

      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-on-surface-variant mb-8">
        <button onClick={() => router.push('/admin/courses')} className="hover:text-primary transition-colors">
          Admin Courses
        </button>
      ...
      <h1 className="text-4xl font-headline font-black text-primary mb-2">{course.title}</h1>
      <p className="text-on-surface-variant mb-10">{course.description}</p>

      {/* Add Module */}
      <div className="bg-white rounded-2xl border border-outline-variant/10 p-6 mb-8 shadow-sm">
        <h2 className="font-headline font-bold text-primary mb-4">Add Module</h2>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-xl border border-outline-variant/20 focus:outline-none focus:border-primary"
          ...
          <button
            ...
            className="bg-primary text-on-primary px-6 py-3 rounded-xl font-bold hover:opacity-90 transition-all disabled:opacity-60"
          >
          ...
          .map((mod: any, mIdx: number) => (
            <div key={mod.id} className="bg-white rounded-2xl border border-outline-variant/10 overflow-hidden shadow-sm">
              <div className="px-6 py-4 bg-surface-container-low border-b border-outline-variant/10 flex items-center gap-3">
                ...
                <h3 className="font-headline font-bold text-primary flex-1">{mod.title}</h3>
                <span className="text-xs text-on-surface-variant">{mod.topics?.length || 0} topics</span>
              </div>
              ...
              {/* Add Topic */}
              <div className="px-6 py-4 bg-surface-container-lowest border-t border-outline-variant/10 flex gap-3">
                <input
                  type="text"
                  value={topicInputs[mod.id] || ''}
                  onChange={(e) => setTopicInputs((prev) => ({ ...prev, [mod.id]: e.target.value }))}
                  placeholder="Add topic title..."
                  className="flex-1 px-4 py-2 rounded-xl border border-outline-variant/20 focus:outline-none focus:border-primary text-sm"
                ...
                <button
                  ...
                  className="bg-primary text-on-primary px-4 py-2 rounded-xl font-bold text-sm hover:opacity-90 transition-all disabled:opacity-40"
                >
                  Add Topic
                </button>
              </div>
